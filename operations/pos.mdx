...
description: "Use the POS screen to find customers, collect payment for outstanding invoices across all methods, and release packages — all from a single screen."
---

The Point of Sale (POS) screen is your primary tool for face-to-face transactions. From here you can find a customer, see all of their outstanding invoices and paid packages, collect payment using any accepted method, and immediately release their packages — without leaving the screen. Every action is tied to a customer session, so you always know exactly who you're serving and what they owe.
...
<Steps>
  ...
  <Step title="Confirm the customer is loaded">
    The modal closes and the customer's stats panel, unpaid invoices, and paid packages all appear. The button label changes to **Switch Customer** for the rest of the session.
  </Step>
</Steps>
...
<CardGroup cols={4}>
  <Card title="Customer Profile" icon="user">
    Shows the customer's full name and their mailbox/account address code (e.g. SHP-0042). Click the name to open their full customer profile in a new tab.
  </Card>
...
<Steps>
  ...
  <Step title="Click Complete Transaction">
    The payment is processed. A success screen displays the receipt ID, transaction reference, total collected, and any change given. Click **Print Receipt** to open the thermal receipt in a new tab, which will print automatically.
  </Step>
...
After every successful transaction the system issues a **Receipt ID** (e.g. `RCP-00123`). This ID links to a printable thermal receipt that shows:
